...
On **River Classic**, workspaces represent all 32 tags on each output. Multiple tags can be active at once, and occupied
and urgent states follow the compositor's tag masks. Clicking a tag focuses its output and displays only that tag.
Set `hide_when_empty = true` to hide unused tags while keeping active and urgent tags visible. River Classic does not
provide per-window tag assignments, so per-workspace application lists and taskbar workspace grouping are unavailable.
